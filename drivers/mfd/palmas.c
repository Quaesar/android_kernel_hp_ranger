...
extern int tegra_pinmux_set_tristate(int pg, enum tegra_tristate tristate);
#define 	TEGRA_PINGROUP_GMI_CS3_N  60
u8 usb1_mode_is_host=0;
u8 send_aok_is_ok=0;
u8 g_is_wifi_wake=0;
...
static int __init usb1_mode(char *str)
{
	printk("USB_HOST=%s\n", str);
...
__setup("usb1_mode=", usb1_mode);

char get_usb1_mode(void)
{
       return usb1_mode_is_host;
}
EXPORT_SYMBOL_GPL(get_usb1_mode);

#if RANGER_TEST_HUB1
void set_hub1_disable(int disable)
{
	if(disable==1){
...
EXPORT_SYMBOL_GPL(set_hub1_disable);
#endif

#if RANGER_TEST_HUB2
void set_hub2_disable(int disable)
{
	if(disable==1){
		gpio_set_value(TEGRA_GPIO_PU3, 0);
	}else{
		gpio_set_value(TEGRA_GPIO_PU3, 1);
	}			
}	
EXPORT_SYMBOL_GPL(set_hub2_disable);
#endif
#if 1 //RANGER_TEST_USB1
void set_usb1_id_status_to_tristate(int flag)
{
     if(flag==0){
	tegra_pinmux_set_tristate(TEGRA_PINGROUP_GMI_CS3_N, TEGRA_TRI_NORMAL); 
      }else{
     	tegra_pinmux_set_tristate(TEGRA_PINGROUP_GMI_CS3_N, TEGRA_TRI_TRISTATE); 
     }

}
EXPORT_SYMBOL_GPL(set_usb1_id_status_to_tristate);

void set_usb1_vbus_en(int flag)
{
	if(flag==0){
...
EXPORT_SYMBOL_GPL(set_usb1_vbus_en);

void set_usb1_host(int flag)
{
	if(flag==1){
...
EXPORT_SYMBOL_GPL(set_usb1_host);
#endif
//display
int get_S50_status(void)
{
        int ret=0;
	ret=gpio_get_value(TEGRA_GPIO_PS2);
	return ret;
}
EXPORT_SYMBOL_GPL(get_S50_status);

int get_S51_status(void)
{
        int ret=0;
	ret=gpio_get_value(TEGRA_GPIO_PS1);
	return ret;
}
EXPORT_SYMBOL_GPL(get_S51_status);

void set_S50_status(int disable)
{
    if(send_aok_is_ok==1)
    {
	if(disable==0){	
		      gpio_set_value(TEGRA_GPIO_PS2, 0);	
	}else{
                if(gpio_get_value(TEGRA_GPIO_PS1) == 0)
                            gpio_set_value(TEGRA_GPIO_PS1, 1);
		gpio_set_value(TEGRA_GPIO_PS2, 1);
	}
	printk("set_S50_status-------[%d] \r\n",disable);
    }
}	
EXPORT_SYMBOL_GPL(set_S50_status);
//wake
void set_S51_status(int disable)
{
	if(disable==0){
		set_S50_status(0);			
		gpio_set_value(TEGRA_GPIO_PS1, 0);
	}else{
	        if(g_is_wifi_wake == 0)
	        {
		     gpio_set_value(TEGRA_GPIO_PS1, 1);
		...
		{
		      printk(" Is wifi wake,S51 status[%d] \r\n",get_S50_status()); 
		      //g_is_wifi_wake=0;	
		      goto GPIO_S51_end;
		}
	}
	printk("set_S51_status-------[%d] \r\n",disable);
GPIO_S51_end:
	return;
}	
EXPORT_SYMBOL_GPL(set_S51_status);
//wake
void enable_S50(int flag)
{
	if(flag==0){
		send_aok_is_ok=0;
	}else{
			    ...
			    const struct i2c_device_id *id)
{
	...
	gpio_request(TEGRA_GPIO_PR6, "hub_pwr_en");
	gpio_set_value(TEGRA_GPIO_PR6, 1);
	gpio_direction_output(TEGRA_GPIO_PR6, 1);	
	
	gpio_request(TEGRA_GPIO_PW3, "hub1_rest");
	gpio_set_value(TEGRA_GPIO_PW3, 1);
	gpio_direction_output(TEGRA_GPIO_PW3, 1);
#if RANGER_TEST_HUB2
	gpio_request(TEGRA_GPIO_PU3, "hub2_rest");
	gpio_set_value(TEGRA_GPIO_PU3, 1);
	gpio_direction_output(TEGRA_GPIO_PU3, 1);
#endif
	gpio_request(TEGRA_GPIO_PS2, "s_gpio_50");
	gpio_set_value(TEGRA_GPIO_PS2, 1);
	gpio_direction_output(TEGRA_GPIO_PS2, 1);
	
	gpio_request(TEGRA_GPIO_PS1, "s_gpio_51");
	gpio_set_value(TEGRA_GPIO_PS1, 1);
	gpio_direction_output(TEGRA_GPIO_PS1, 1);

#if RANGER_TEST_USB1
        if (usb1_mode_is_host ==1)
        {
           //USB1 fixed to the host mode
...
{
	printk("palmas_suspen+++++++ \r\n");
	gpio_set_value(TEGRA_GPIO_PR6, 0);
	K2U_CMD(0,1);
	//set_S51_status(0);
	return 0;
}
