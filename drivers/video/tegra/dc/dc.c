...
void wolo_tegra_dc_disable(struct tegra_dc *dc);
...
void tegra_dc_enable(struct tegra_dc *dc)
{
	...
	mutex_unlock(&dc->lock);
	trace_display_mode(dc, &dc->mode);
	dev_warn(&dc->ndev->dev,"tegra_dc_enable...\n");
	if(g_crypt_control_bl==1)
	{
	      set_S50_status(1);
	      printk("[FW]crypt_control 1\r\n");	  
	      g_crypt_control_bl=0;	  
	}
}
...
{
     //rename tegra_dc_disable to wolo_tegra_dc_disable 
     //if(g_into_mode ==0)
     //{
              //set_S50_status(0);
	      //g_enable_s50=0;			
     //}
     dev_warn(&dc->ndev->dev,"tegra_dc_disable...\n");
     wolo_tegra_dc_disable(dc);	 
}
...
#if 0	// wolo: remove for fix powe-off screen garbage
	tegra_dc_disable(dc);
#endif
}
