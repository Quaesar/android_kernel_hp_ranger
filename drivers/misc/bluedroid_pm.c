...
static int bluedroid_pm_rfkill_set_power(void *data, bool blocked)
{
	...
	printk("bluedroid_pm_rfkill_set_power, block=%d, get_value=%d, gpio_shutdown=%d\n", blocked, 
		                                            gpio_get_value(bluedroid_pm->gpio_shutdown),
	...
	if (blocked) {
		if (bluedroid_pm->gpio_shutdown){
	             printk("bluedroid_pm_rfkill_set_power, off\n"); 
			gpio_set_value(bluedroid_pm->gpio_shutdown, 0);
		}
		//Wifi SDIO Reset pin (MACALLAN_WLAN_RST)
		if(gpio_get_value(TEGRA_GPIO_PCC5)==0){
	             printk("bluedroid_pm_rfkill_set_power, wifi reset pin low ==> pull low PWR_EN\n");
			mdelay(50);
                   // wifi reset pin low ==> pull high WF_PWR_EN
			gpio_set_value(TEGRA_GPIO_PX7, 0);
		}else{
	             printk("bluedroid_pm_rfkill_set_power, wifi reset pin high  ==> don't pull low PWR_EN\n");
                   // wifi reset pin high  ==> don't pull low WF_PWR_EN
		}
		...
		//Wifi SDIO Reset pin (MACALLAN_WLAN_RST)
		if(gpio_get_value(TEGRA_GPIO_PCC5)==0){
	             printk("bluedroid_pm_rfkill_set_power, wifi reset pin low ==> pull high PWR_EN\n");
                   // wifi reset pin low ==> pull high WF_PWR_EN
			gpio_set_value(TEGRA_GPIO_PX7, 1);
			mdelay(100);
		}else{
	             printk("bluedroid_pm_rfkill_set_power, wifi reset pin high  ==> don't pull high PWR_EN\n");
                   // wifi reset pin high  ==> don't pull high WF_PWR_EN
		}
		if (bluedroid_pm->vdd_3v3)
		...
		if (bluedroid_pm->gpio_shutdown){
	             printk("bluedroid_pm_rfkill_set_power, on\n"); 
...
static int bluedroid_pm_probe(struct platform_device *pdev)
{
	...
	printk("bluedroid_pm_probe, get_resource--shutdown_gpio\n"); 
	if (res) {
		bluedroid_pm->gpio_shutdown = res->start;
	       printk("bluedroid_pm_probe, bluedroid_pm->gpio_shutdown=%d\n", bluedroid_pm->gpio_shutdown); 
