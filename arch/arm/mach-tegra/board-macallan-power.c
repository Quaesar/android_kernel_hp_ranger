...
static struct regulator_consumer_supply palmas_smps8_supply[] = {
	...
	REGULATOR_SUPPLY("avdd_usb_pll", "tegra-ehci.2"),//test
	//REGULATOR_SUPPLY("avddio_usb", "tegra-ehci.2"),	//test
...
PALMAS_REGS_PDATA(smps6, 0,  0, NULL, 0, 0, 1, NORMAL,
	0, 0, 0, 0, 0);
...
PALMAS_REGS_PDATA(ldo9, 1800,  2900, palmas_rails(smps9), 0, 0, 1, 0,
	0, 0, 0, 0, 0);
PALMAS_REGS_PDATA(ldoln, 3300,   3300, NULL, 1, 1, 1, 0,
	0, 0, 0, 0, 0);
...
// Wolo: start-- Wifi_BT
struct palmas_clk32k_init_data palmas_clk32k_wifi[] = {
	{
...
// Wolo: start-- Wifi_BT
	.clk32k_init_data =  palmas_clk32k_wifi,
	.clk32k_init_data_size = ARRAY_SIZE(palmas_clk32k_wifi),
// Wolo: end-- Wifi_BT
	.use_power_off = true,
	.pinctrl_pdata = &palmas_pinctrl_pdata,
	.extcon_pdata = &palmas_extcon_pdata,
	.long_press_delay = PALMAS_LONG_PRESS_KEY_TIME_12SECONDS,
};
...
//wolong static struct regulator_consumer_supply fixed_reg_vd_cam_1v8_supply[] = {
static struct regulator_consumer_supply fixed_reg_vd_cam_3v3_supply[] = {
...
FIXED_REG(6,	vd_cam_3v3,	vd_cam_3v3,
	palmas_rails(smps8),	0,	0,
...
static struct regulator *CAM_LDO1_EN_3v3_cam = NULL;	
        ...
        if (CAM_LDO1_EN_3v3_cam == NULL) {
            ...
            if (WARN_ON(IS_ERR(CAM_LDO1_EN_3v3_cam))) {
				//pr_err
                printk(" b> ======= b>%s: couldn't get regulator vdd_cam_3v3: %ld\n",
                    __func__, PTR_ERR(CAM_LDO1_EN_3v3_cam));
                return;
             }
         }
        regulator_enable(CAM_LDO1_EN_3v3_cam);
}
...
int ret;
	...
	ret = platform_add_devices(fixed_reg_devs,
			ARRAY_SIZE(fixed_reg_devs));

	macallan_camera_power_on_for_mic(); //b+
